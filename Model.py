...
class Lang:
    ...
    # need to ensure space exists between ever character
...
raw_data_file_path = ('spacedata100k2.txt',)
...
"""**Before running next cell**, to prevent runtime disconnect:
1. Ctrl-Shift-i
2. Open Browser Console
3. Paste below Function and execute
...
Learning Rate: 1 
Time: 2h 17m 9s 
Train Loss: 0.004738005296083597
Test set loss: 0.0097538337799219
"""
...
input_lang, output_lang, train_pairs, test_pairs = prepareData(
    ...
    start_filter=start_filter, perc_train_set=perc_train_set)

# input_lang = Lang('en')
# output_lang = Lang('java')
...
pref = 'final-%sLayers-%shidden-%sdropout-%sbatchsize' % (layers, hidden_size, dropout, batch_size)

if use_cuda:
    print('Cuda being used')
    decoder.load_state_dict(torch.load(pref + '_dec_weights.pt'))
    encoder.load_state_dict(torch.load(pref + '_enc_weights.pt'))
    encoder = encoder.cuda()
    decoder = decoder.cuda()
else:
    print('Cuda is unavailable')
    decoder.load_state_dict(torch.load(pref + '_dec_weights.pt', map_location=torch.device('cpu')))
    encoder.load_state_dict(torch.load(pref + '_enc_weights.pt', map_location=torch.device('cpu')))


def run(text):
    outside_sent = normalizeString(text)
    return evaluate(encoder, decoder, outside_sent, cutoff_length=100)
